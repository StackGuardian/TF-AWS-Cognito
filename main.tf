...
resource "aws_cognito_user_pool" "user_pool" {
    ...
    name = coalesce(var.user_pool_name, module.this.id)  
    alias_attributes         = var.alias_attributes != null ? var.alias_attributes : ["email","preferred_username",]
    ...
    dynamic "device_configuration" {
        for_each = contains(["ALWAYS", "USER_OPT_IN"], var.user_device_tracking) ? [true] : []
    ...
    dynamic "schema" {
        ...
        developer_only_attribute = attribute.value.developer_only_attribute
        mutable                  = attribute.value.mutable
...
locals {
  clients = {
    for client in var.clients : replace(lower(client.name), "/[^a-z0-9]/", "-") => {
      ...
      refresh_token_validity               = lookup(client, "refresh_token_validity", null)
      ...
      access_token_validity                = lookup(client, "access_token_validity", null)
      id_token_validity                    = lookup(client, "id_token_validity", null)
      token_validity_units                 = lookup(client, "token_validity_units", null)
      enable_token_revocation              = lookup(client, "enable_token_revocation", null)
    }
  }
}
